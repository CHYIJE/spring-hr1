...
public class MainService {
    ...
    public void deleteMainData(Integer regionId){
        RegionsEntity regionEntity = regionsRepository.findByRegionId(regionId);

        if(regionEntity == null) {
            throw new RuntimeException("이미 삭제된 지역입니다.");
        }

        regionsRepository.delete(regionEntity);
    ...
    public ResUpdateMainDTO getUpdateMainPageDate(Integer regionsId){
        RegionsEntity regionEntity = regionsRepository.findByRegionId(regionsId);

        return ResUpdateMainDTO.fromEntity(regionEntity);
    }
